...
class UmoBrokerage(ReportModel):
    ...
    def _carry_operations(self) -> bool:
        print(f'UmoBrokerage: _carry_operations(stage={self.stage})')

        ext_dataframe = self.make_dataframe_from_file(self.path_ext, self.data_folder_report_path)
        ext_dataframe = self.set_colum_names({0: "numer_umowy", 1: "numer_klienta", 2: 'typ_atrybutu', 3: 'wartosc'},
                                             ext_dataframe)
        if self.stage == TextEnum.LOAD:
            src_dataframe = self.make_dataframe_from_file(self.path_src, self.data_folder_report_path)
            src_dataframe = self.set_colum_names({0: "numer_umowy", 1: "numer_klienta"}, src_dataframe)
        ...
        if self.stage == TextEnum.END:
            ...
            tgt_dataframe = self.set_colum_names({0: "numer_umowy", 1: "numer_klienta", 2: 'typ_atrybutu', 3: 'wartosc'}, tgt_dataframe)
    ...
    def convert_src(dataframe: pandas.DataFrame) -> pandas.DataFrame:
        ProgresBarStatus.increase()
        return dataframe[["numer_umowy", "numer_klienta"]]
    ...
    def create_report(self) -> TextEnum | bool:
        ...
        print(f"UmoBrokerage(): create_report({self.path_excel}  {self.save_folder_report_path})")
        try:
            ...
            print(f"UmoBrokerage(): create_report  Error tworzenia excela : {e}")
        ...
        try:
            if self.stage == TextEnum.LOAD:
                f2f = excel_workbook.create_f2f_report(
                    ...
                    merge_on_cols=["numer_umowy"],
                    compare_cols=["numer_klienta"],
                    text_description="Rejestr umów maklerskich")
            ...
            print(f"UmoBrokerage(): create_report  Error tworzenia raportu : {e}")
            dispatcher.send(signal=UPDATE_TEXT_SIGNAL, message=f"Błąd tworzenia raportu {e}", head='error')
            return TextEnum.CREATE_ERROR

        try:
            excel_workbook.save_to_excel({f"f2f_brokerage_agreement": f2f}, merge_on=["numer_umowy"])
        except Exception as e:
            print(f"UmoBrokerage(): create_report  Error zapisywania raportu : {e}")
        ...
        try:
            ...
            print(f"UmoBrokerage(): add_password_to_excel  Error daodawania hasła do raportu : {e}")
