...
class RachunkiPrzelewy(ReportModel):
    ...
    def _carry_operations(self) -> bool:
        print(f'RachunkiPrzelewy: _carry_operations(stage={self.stage})')

        ext_dataframe = self.make_dataframe_from_file(self.path_ext, self.data_folder_report_path)
        ext_dataframe = self.set_colum_names({0: "rachunek", 2: "id_obcy", 1: "rachunek_nrb"}, ext_dataframe)
        if self.stage == TextEnum.LOAD:
            src_dataframe = self.make_dataframe_from_file(self.path_src, self.data_folder_report_path)
            src_dataframe = self.set_colum_names({0: "rachunek", 1: "rachunek_nrb", 2: "id_obcy"}, src_dataframe)
        ...
        if self.stage == TextEnum.END:
            ...
            tgt_dataframe = self.set_colum_names({0: "rachunek", 2: "id_obcy", 1: "rachunek_nrb"}, tgt_dataframe)
    ...
    def convert_src(dataframe: pandas.DataFrame) -> pandas.DataFrame:
        dataframe['rach_nrb'] = dataframe['rachunek_nrb']
        return dataframe[['rachunek', 'rach_nrb', 'rachunek_nrb']]
    ...
    def create_report(self) -> TextEnum | bool:
        ...
        print(f"RachunkiPrzelewy(): create_report({self.path_excel}  {self.save_folder_report_path})")
        try:
            ...
            print(f"RachunkiPrzelewy(): create_report  Error tworzenia excela : {e}")
        ...
        try:
            if self.stage == TextEnum.LOAD:
                f2f = excel_workbook.create_f2f_report(
                    ...
                    merge_on_cols=["rachunek", "rach_nrb"],
                    compare_cols=["rachunek_nrb"],
                    text_description="Kartoteka rachunków bankowych na które dopuszcza się wystawienie przelewów")
            ...
            print(f"RachunkiPrzelewy(): create_report  Error tworzenia raportu : {e}")
            dispatcher.send(signal=UPDATE_TEXT_SIGNAL, message=f"Błąd tworzenia raportu {e}", head='error')
            return TextEnum.CREATE_ERROR

        try:
            excel_workbook.save_to_excel({f"f2f_rachunki_przelewy": f2f}, merge_on=["rachunek", "rach_nrb"])
        except Exception as e:
            print(f"RachunkiPrzelewy(): create_report  Error zapisywania raportu : {e}")
        ...
        try:
            ...
            print(f"RachunkiPrzelewy(): add_password_to_excel  Error dodawania hasła do raportu : {e}")
